...
BTNode* simulate(BTNode *root)
{
	if (root != NULL) {
		switch (root->token) {
        case ID:
            if(strcmp(root->lexeme, "x")&&strcmp(root->lexeme, "y")&&strcmp(root->lexeme, "z"))
                error();
            else count[root->lexeme[0]-'x']++;
            break;
        case ASSIGN:
            error();
            break;
...
void statement(void)
{
    ...
    else
    {
        ...
        else if(retp->left->token != ID || retp->left->left || retp->left->right) error();
        if (match(END))
        {
